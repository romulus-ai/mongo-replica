...
originalArgOne="$1"
echo "Running entrypoint.sh with ${originalArgOne}"
ps aux
...
if [[ "$originalArgOne" == mongo* ]] && [ "$(id -u)" = '0' ]; then
    ...
    echo "exec gosu mongodb $BASH_SOURCE $@"
...
if [ "$originalArgOne" = 'mongod' ]; then
    ...
    echo "Ensure Credentials are set..."
    shouldPerformInitdb=
    if [ "$MONGO_INITDB_ROOT_USERNAME" ] && [ "$MONGO_INITDB_ROOT_PASSWORD" ]; then
        ...
        echo
        echo " error: missing 'MONGO_INITDB_ROOT_USERNAME' or 'MONGO_INITDB_ROOT_PASSWORD'"
        echo "   both must be specified for a user to be created"
        echo
        exit 1
    fi
    ...
    if [ -z "$shouldPerformInitdb" ]; then
        ...
        echo "We should perfom initdb with: $shouldPerformInitdb..."
    fi
    ...
    if [ -n "$shouldPerformInitdb" ]; then
        dbPath="$(_dbPath "$@")"
        echo "Check Filesystem in $dbPath"
        ls -la $dbPath
        for path in \
            "$dbPath/WiredTiger" \
            "$dbPath/journal" \
        ...
        ; do
            if [ -e "$path" ]; then
                shouldPerformInitdb=
                echo "Init abort, already initialized..."
                break
            fi
        done
    fi
    ...
    if [ -n "$shouldPerformInitdb" ]; then
        ...
        echo "Starting mongoDB with ${mongodHackedArgs[@]} --fork"
...
EOJS
        ...
        # MODIFIED: INIT replicaset
        echo "Initializing Replicaset..."
        echo "numactl --interleave=all $@ --tlsMode disabled --logpath /proc/1/fd/1 --logappend --pidfilepath "$pidfile" --fork"

        numactl --interleave=all "$@" --tlsMode disabled --logpath /proc/1/fd/1 --logappend --pidfilepath "$pidfile" --fork
        # check to see that our "mongod" actually did start up (catches "--help", "--version", MongoDB 3.2 being silly, slow prealloc, etc)
        # https://jira.mongodb.org/browse/SERVER-16292
        tries=30
        while true; do
        ...
        "${mongo[@]}" --username admin --password adminpassword admin <<-EOJS
            rs.initiate()
EOJS
